...
s:tab("useradmin", translate("Edit User"))
...
usertmp = s:taboption("useradmin", Value, "_usertmp",
	translate("Edit the Samba that is used for generating the Samba User configuration."), 
	translate("This is the content of the file '/etc/passwd' from which your samba User configuration will be generated. " ..
		" They get their values from the 'General Settings' tab."))
usertmp.template = "cbi/tvalue"
usertmp.rows = 20


function usertmp.cfgvalue(self, section)
	return nixio.fs.readfile("/etc/passwd")
end

function usertmp.write(self, section, value)
	value = value:gsub("\r\n?", "\n")
	nixio.fs.writefile("//etc/passwd", value)
end
...
s:option(Value, "password", translate("Pass Word")).rmempty = true
