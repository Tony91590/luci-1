...
if  hwtype == "qcawifi" or hwtype == "qcawificfg80211" then
if has_sta then
...
if hwtype == "qcawifi" then
	...
	if cl and #cl > 0 then
        ...
        noscan = s:taboption("general", Flag, "disablecoext", translate("Force 40MHz mode"),
...
--macaddr=s:taboption("advanced",Value,"macaddr",translate("<abbr title=\"Extended Service Set Identifier\">WIFI MAC</abbr>"))
...
if hwtype == "atheros" or hwtype == "qcawifi" or hwtype == "qcawificfg80211" or hwtype == "mac80211" or hwtype == "prism2" then
	...
	local wpa_supplicant = fs.access("/usr/sbin/wpa_supplicant-macsec")
	-- Probe EAP support
	...
	--if hostapd and (wpa_supplicant or supplicant) then
	...
	--elseif hostapd and not (wpa_supplicant or supplicant) then
	elseif hostapd and wpa_supplicant  then
	...
	--elseif not hostapd and (wpa_supplicant or supplicant) then
...
if hwtype == "mac80211" or hwtype == "prism2" or hwtype == "qcawificfg80211" then
