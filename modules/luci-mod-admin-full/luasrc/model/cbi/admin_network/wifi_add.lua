...
function newnet.parse(self, section)
	...
	if m.hidden.device=="wifi0"then
	net = nw:add_network(value, { ifname = "ath01" })
	elseif m.hidden.device=="wifi1"then
	net = nw:add_network(value, { ifname = "ath11" })
	else
	net = nw:add_network(value, { ifname = "ath21" })
	end
